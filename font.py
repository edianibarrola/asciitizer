...
def transform_image_to_ascii(image_path=INPUT_IMAGE_PATH, characters_per_row=ASCII_CHARACTERS_IN_ROW, height_width_ratio=HEIGHT_WIDTH_ADJUSTMENT, color_enabled=False, black_canvas=False):
    ...
    if color_enabled:  
        ascii_str = convert_pixels_to_ascii_with_color(image)
    else:
...
def transform_ascii_to_image(ascii_text, font_size=ASCII_FONT_SIZE, height_width_ratio=HEIGHT_WIDTH_ADJUSTMENT, color_enabled=False, black_canvas_enabled=False):
    ...
    if color_enabled:
        ...
        img = Image.new('RGB', (max_width, total_height), color=(0, 0, 0))  # Set canvas to black for black_canvas_enabled
    ...
    for line in ascii_str_lines:
        line_width, line_height = calculate_text_dimensions(line, fnt)
        if color_enabled:
            d.text(((max_width - line_width) // 2, current_height), line, font=fnt, fill=(0, 0, 0))
        else:
            d.text(((max_width - line_width) // 2, current_height), line, font=fnt, fill=(255, 255, 255))  # Set text to white for black_canvas_enabled
...
def main(image_path=INPUT_IMAGE_PATH, characters_per_row=ASCII_CHARACTERS_IN_ROW, height_width_ratio=HEIGHT_WIDTH_ADJUSTMENT):
    render_option = input("Please choose your desired ASCII art format:\n"
                          "Enter 'color' for color,\n"
                          "Enter 'grayscale' for black and white: ").strip().lower()
    
    color_enabled = black_canvas_enabled = False
    ...
    output_option = input("Please choose your desired output format:\n"
                          "Enter 'black_canvas' for black canvas with white characters,\n"
                          "Enter 'white_canvas' for white canvas with black characters: ").strip().lower()

    if output_option == 'black_canvas':
        black_canvas_enabled = True
    elif output_option == 'white_canvas':
        black_canvas_enabled = False
    else:
        print("Invalid input. Defaulting to white canvas with black characters.")
        black_canvas_enabled = False

    ascii_img = transform_image_to_ascii(image_path, characters_per_row, height_width_ratio, color_enabled, black_canvas_enabled)
    img = transform_ascii_to_image(ascii_img, font_size=ASCII_FONT_SIZE, height_width_ratio=height_width_ratio, color_enabled=color_enabled)
